...
def make_multiwave_envelope(kpoints: jnp.ndarray) -> envelopes.Envelope:
    ...
    def apply(
        *, ae: jnp.ndarray, r_ae: jnp.ndarray, r_ee: jnp.ndarray, sigma: jnp.ndarray
    ) -> jnp.ndarray:
        """See ferminet.envelopes.EnvelopeApply."""
        del r_ae, r_ee  # unused
        phase_coords = ae @ kpoints.T
        waves = jnp.concatenate((jnp.cos(phase_coords), jnp.sin(phase_coords)), axis=2)
        env = waves @ (sigma**2.0)
...
def make_kpoints(
    ...
    max_k = int(jnp.ceil(min_kpoints * dk) ** (1 / 3.0))
