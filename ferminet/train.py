...
def init_electrons(  # pylint: disable=dangerous-default-value
    ...
    niter = 0
    total_electrons = sum(
        atom.charge - core_electrons.get(atom.symbol, 0) for atom in molecule
    )
    ...
    electron_spins = _assign_spin_configuration(electrons[0], electrons[1], batch_size)
...
def make_opt_update_step(
    evaluate_loss: qmc_loss_functions.LossFn, optimizer: optax.GradientTransformation
) -> OptUpdate:
...
def make_kfac_training_step(
    ...
    shared_mom = constants.replicate_all_local_devices(
        jnp.zeros([]), axis_name=constants.PMAP_AXIS_NAME
    )
    shared_damping = constants.replicate_all_local_devices(
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
    ...
    batch_atoms = constants.replicate_all_local_devices(
        batch_atoms, axis_name=constants.PMAP_AXIS_NAME
    )
    batch_charges = jnp.tile(charges[None, ...], [device_batch_size, 1])
    batch_charges = constants.replicate_all_local_devices(
    ...
    params = constants.replicate_all_local_devices(
    ...
    if cfg.system.get("states", 0):
        if cfg.optim.objective == "vmc_overlap":
            logabs_network = networks.make_state_trace(
                signed_network, cfg.system.states
            )
        else:
            logabs_network = utils.select_output(
                networks.make_total_ansatz(
                    signed_network,
    ...
    if cfg.system.get("states", 0):
        if cfg.optim.objective == "vmc_overlap":
            ...
            log_network_for_loss = networks.make_state_matrix(
                signed_network, cfg.system.states
            )

            def log_network(*args, **kwargs):
                phase, mag = log_network_for_loss(*args, **kwargs)
                return mag + 1.0j * phase
        else:

            def log_network(*args, **kwargs):
                if not use_complex:
                    raise ValueError(
                        "This function should never be used if the "
                        "wavefunction is real-valued."
                    )
                meta_net = networks.make_total_ansatz(
                    signed_network, cfg.system.get("states", 0), complex_output=True
                )
    ...
    ckpt_restore_filename = checkpoint.find_last_checkpoint(
        ckpt_save_path
    ) or checkpoint.find_last_checkpoint(ckpt_restore_path)

    if ckpt_restore_filename:
        (t_init, data, params, opt_state_ckpt, mcmc_width_ckpt, density_state_ckpt) = (
    ...
    if cfg.observables.s2:
        observable_fns["s2"] = observables.make_s2(
            signed_network, nspins, states=cfg.system.states
        )
        observable_states["s2"] = None
        train_schema += ["s2"]
        if cfg.system.states:
            s2_matrix_file = open(os.path.join(ckpt_save_path, "s2_matrix.npy"), "ab")
    if cfg.observables.dipole:
        observable_fns["dipole"] = observables.make_dipole(
            signed_network, states=cfg.system.states
        )
    ...
    if cfg.observables.density:
        (observable_states["density"], density_update, observable_fns["density"]) = (
            observables.make_density_matrix(
        ...
        pmap_density_axes = observables.DensityState(
            t=None,
            positions=0,
            probabilities=0,
            move_width=0,
        ...
        pmap_fn = constants.pmap(
            observable_fns["density"], in_axes=(0, 0, pmap_density_axes)
        )
    ...
    if cfg.system.make_local_energy_fn:
        if laplacian_method != "default":
            raise NotImplementedError(
                f"Laplacian method {laplacian_method}"
                "not yet supported by custom local energy fns."
            )
        if cfg.optim.objective == "vmc_overlap":
            raise NotImplementedError(
                "Overlap penalty not yet supported for customlocal energy fns."
            )
    ...
    if cfg.optim.get("spin_energy", 0.0) > 0.0:
        ...
        local_s2_fn = observables.make_s2(
            signed_network, nspins=nspins, states=cfg.system.states
        )
    ...
    if cfg.optim.objective == "vmc":
        ...
        if cfg.optim.overlap.weights is None:
            overlap_weight = tuple([1.0 / (1.0 + x) for x in range(cfg.system.states)])
    ...
    if cfg.optim.optimizer == "none" and opt_state_ckpt is not None:
        ...
        if cfg.system.states > 0 and "state_scale" not in params:
            state_matrix = utils.select_output(
                networks.make_state_matrix(signed_network, cfg.system.states), 1
            )
            ...
            if isinstance(params, dict):  # Always true, but prevents type errors
                params[
                    "state_scale"
                ] = -state_scale  # pytype: disable=unsupported-operands
    ...
    with writer_manager as writer:
        ...
        for t in range(t_init, cfg.optim.iterations):
            ...
            if t % cfg.log.stats_frequency == 0:
                logging_str = (
                    "Step %05d: %03.4f E_h, exp. variance=%03.4f E_h^2, pmove=%0.2f"
                )
