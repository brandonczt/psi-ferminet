...
def mh_block_update(
    ...
    if atoms is None:  # symmetric prop, same stddev everywhere
        x2 = x1.at[:, ii].add(stddev * jax.random.normal(subkey, shape=x1[:, ii].shape))
        ...
        raise NotImplementedError(
            "Still need to work out reverse probabilities for asymmetric moves."
        )
...
def make_mcmc_step(
    batch_network, batch_per_device, steps=10, atoms=None, ndim=3, blocks=1
):
