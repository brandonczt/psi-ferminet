...
def make_loss(
    ...
    vmap = (
        ...
        else functools.partial(folx.batched_vmap, max_batch_size=max_vmap_batch_size)
    )
    ...
    def total_energy_jvp(primals, tangents):  # pylint: disable=unused-variable
        ...
        if complex_output:
            ...
            term1 = jnp.dot(clipped_el, jnp.conjugate(psi_tangent)) + jnp.dot(
                jnp.conjugate(clipped_el), psi_tangent
            )
...
def make_energy_overlap_loss(
    ...
    def total_energy_and_overlap(
        ...
        loss_diff = e_l - loss
        variance = constants.pmean(jnp.mean(loss_diff * jnp.conj(loss_diff), axis=0))
        ...
        sign_psi, log_psi = batch_network(
        ...
        return (
            weighted_energy + overlap_penalty * total_overlap,
            AuxiliaryLossData(
                energy=loss,
                variance=variance.real,
                local_energy=e_l,
    ...
    def total_energy_and_overlap_jvp(primals, tangents):  # pylint: disable=unused-variable
        ...
        if complex_output:
            raise NotImplementedError(
                "Complex output is not supported with penalty "
        ...
        overlap_diff = 2 * jnp.sum(
            jnp.triu(clipped_overlap * overlap_diff.transpose((0, 2, 1)), 1), axis=1
        )
        ...
        kfac_jax.register_normal_predictive_distribution(jax.vmap(jnp.diag)(log_primal))
        device_batch_size = jnp.shape(aux_data.local_energy)[0]
        tangent_loss = energy_diff * overlap_weight + overlap_penalty * overlap_diff
        tangents_out = (
            jnp.sum(jax.vmap(jnp.diag)(log_tangent) * tangent_loss) / device_batch_size,
            aux_data,
        )
