...
Observable = Callable[
    [networks.ParamTree, networks.FermiNetData, Optional[DensityState]], jnp.ndarray
]

DensityUpdate = Callable[
    [Any, networks.ParamTree, networks.FermiNetData, DensityState], DensityState
]
...
def make_s2(
    ...
    def s2_estimator(
        ...
        if states:
            ...
            sign_psi, log_psi = signed_network(
                params, data.positions, data.spins, data.atoms, data.charges
            )
        ...
        if assign_spin:
            ...
            if states:
                raise NotImplementedError(
                    "S^2 estimation for excited states is only "
...
def make_density_matrix(
    ...
    ckpt_state: Optional[DensityState] = None,
) -> Tuple[DensityState, DensityUpdate, Observable]:
    ...
    if ckpt_state is not None:
        ...
        move_width = constants.replicate_all_local_devices(
    ...
    def density_update(
        ...
        for _ in range(1000 if state.t == 0 else 1):  # burn-in on the first step
            ...
            move_width, pmoves = mcmc.update_mcmc_width(
                state.t,
                state.move_width,
