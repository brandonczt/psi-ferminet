...
def setUpModule():
    ...
    try:
        ...
        logging.info(
            "JAX already initialised so cannot set number of CPU devices. "
...
class QmcPyscfMolTest(parameterized.TestCase):
    ...
    @parameterized.parameters(
        ...
        for mol_string, optimizer, mcmc_blocks in zip(
            MOL_STRINGS, ("adam", "kfac"), (1, 2)
        )
    )
    def test_training_step_pyscf(self, mol_string, optimizer, mcmc_blocks):
        mol = pyscf.gto.Mole()
        mol.build(atom=mol_string, basis="sto-3g", unit="bohr")
