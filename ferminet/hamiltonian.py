...
KineticEnergy = Callable[[networks.ParamTree, networks.FermiNetData], jnp.ndarray]
...
def local_kinetic_energy(
    ...
    if laplacian_method == "default":
        ...
        def _lapl_over_f(params, data):
            ...
            if complex_output:
                ...
                hessian_diagonal = lambda i: (
                    dgrad_f(eye[i])[i] + 1.0j * dgrad_phase(eye[i])[i]
                )
            else:
            ...
            if complex_output:
                ...
                result -= 1.0j * jnp.sum(primal * phase_primal)
            return result
        ...
        def _lapl_over_f(params, data):
            ...
            result = (
                -(output[1].laplacian + jnp.sum(output[1].jacobian.dense_array ** 2))
                / 2
            )
            if complex_output:
                ...
                result -= 1.0j * jnp.sum(
                    output[0].jacobian.dense_array * output[1].jacobian.dense_array
                )
            return result
    else:
        raise NotImplementedError(
            f"Laplacian method {laplacian_method} not implemented."
        )
...
def excited_kinetic_energy_matrix(
    ...
    def _lapl_over_f(params, data):
        ...
        if laplacian_method == "default":
            ...
            lapl = vmap_lapl(
                params, pos_, spins_, data.atoms, data.charges
            )  # K psi_i(r_j) / psi_i(r_j)
        elif laplacian_method == "folx":
            ...
            log_mat = log_out.x
            lapl = (
                -(log_out.laplacian + jnp.sum(log_out.jacobian.dense_array**2, axis=-2))
                / 2
            )
            if complex_output:
                sign_mat = sign_out.x
        ...
        if complex_output:
            psi_mat = jnp.exp(log_mat + 1.0j * sign_mat - jnp.max(log_mat))
        else:
...
def potential_energy(
    r_ae: Array, r_ee: Array, atoms: Array, charges: Array
) -> jnp.ndarray:
...
def local_energy(
    ...
    def _e_l(
        ...
        if states:
            ...
            pot_spectrum = vmap_pot(r_ae, r_ee, data.atoms, effective_charges)[:, None]
            ...
            potential = (
                potential_energy(r_ae, r_ee, data.atoms, effective_charges)
                + pp_local(r_ae)
