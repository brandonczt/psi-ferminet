...
class ElementsTest(parameterized.TestCase):
    ...
    def test_element_group_period(
        self, element, period, group, spin_config, nalpha, nbeta
    ):
        ...
        with self.subTest("Verify spin configuration"):
            if element.period > 5 and (element.group == -1 or 3 <= element.group <= 12):
