...
class Scf:
                    ...
                    tuple(
                        [
                            np.concatenate(
                                [
                                    np.concatenate(
                                        [np.eye(ncore), np.zeros((ncore, n - ncore))],
                                        axis=1,
                                    ),
                                    np.concatenate(
                                        [
                                            np.zeros((n - ncore + 1, ncore)),
                                            np.random.randn(n - ncore + 1, n - ncore)
                                            / np.sqrt(n - ncore),
                                        ],
                                        axis=1,
                                    ),
                                    np.zeros((nbasis - n - 1, n)),
                                ],
                                axis=0,
                            )
                            for n in nelec
    ...
    def mo_coeff(self, mo_coeff):
        ...
        if (
            self.mean_field is not None
            and self.mean_field.mo_coeff is not None
            and self.mean_field.mo_coeff.ndim != mo_coeff.ndim
        ):
            raise ValueError(
                "Attempting to override mo_coeffs with different rank. "
    ...
    def eval_orbitals(
        self, pos: NDArray, nspins: Tuple[int, int]
        ...
        if self.excitations is not None:
            ...
            for excitation in self.excitations:
                ...
                if isinstance(excitation[0], float):
                    ...
                    raise ValueError(
                        f"Unexpected excitation type: {type(excitation[0])}"
                    )
    ...
    def eval_slater(
        self, pos: Union[jnp.ndarray, np.ndarray], nspins: Tuple[int, int]
...
def get_ordered_excitations(
                    ...
                    for spin_occ2 in range(2):
                        for i_occ2 in range(nocc[spin_occ2]):
                            for spin_unocc2 in range(2):
                                for i_unocc2 in range(
                                    nocc[spin_unocc2], norb[spin_unocc2]
                                ):
                                    ...
                                    if (occ_index1 < occ_index2) and (
                                        ...
                                        delta_e = (
                                            ...
                                            - energy[spin_occ1][i_occ1]
                                            - energy[spin_occ2][i_occ2]
                                        )
                                        res.append(
                                            (
                                                delta_e,
    ...
    if len(res) < n:
        raise ValueError(
            "Insufficient single and double excitations. "
