...
def grad_solid_harmonic(r: jnp.ndarray, l_max: int) -> jnp.ndarray:
    ...
    def cartesian_sum(r, ell):
        ...
        if ell == 0:
            return np.array([0.0])
        return np.sum(
            np.stack(np.meshgrid(*[r] * ell, indexing="ij"), axis=0).reshape(ell, -1),
            axis=0,
        )

    dharmonics = []
    for ell in range(l_max + 1):
        dx = cartesian_sum(np.array([1.0, 0.0, 0.0]), ell)
        dy = cartesian_sum(np.array([0.0, 1.0, 0.0]), ell)
        dz = cartesian_sum(np.array([0.0, 0.0, 1.0]), ell)
        d = np.stack([dx, dy, dz], axis=1)
        cp = jax.vmap(functools.partial(cartesian_product, ell=ell), out_axes=-1)
        dcp = jnp.where(
            r.T[None, :, :] == 0, 0, d[:, :, None] * cp(r)[:, None, :] / r.T[None, :, :]
        )
...
def grad_solid_harmonic_by_jacfwd(r: jnp.ndarray, l_max: int) -> jnp.ndarray:
    ...
    for ell in range(l_max + 1):
        ...
        sh = jnp.einsum("cs,cxn->snx", full_cart2sph(ell, True), dcp(r))
        pad = l_max - ell
        sh = jnp.pad(sh, [[pad, pad], [0, 0], [0, 0]])
        dharmonics.append(sh)
    return jnp.stack(dharmonics, axis=1)  # [L, M, grid, xyz]
