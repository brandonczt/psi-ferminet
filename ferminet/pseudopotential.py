...
def project_legendre(
    ...
    if complex_output:
        f_ratio = jnp.exp(
            (out_num[1] - out_denom[1]) + 1.0j * (out_num[0] - out_denom[0])
        )
    else:
...
def make_nonlocal_pseudopotential(
    ...
    vmap_pp_nonloc = vmap_sum(
        pp_nonloc, in_axes=(0, None, None, None, None, 0, 0, 0, 0, None, None, 0)
    )
    ...
    def electron_atom_nonlocal_pseudopotential(key, signed_f, params, data, ae, r_ae):
...
def make_pp_potential(
    ...
    ecp_nwchem_fmt = {
        elements.SYMBOLS[symb].atomic_number: pyscf.gto.basis.load_ecp(ecp, symb)
        for symb in symbols
    }

    n_cores, v_grid_dict, r_grid, n_channels = pp_utils.eval_ecp_on_grid(ecp_nwchem_fmt)
    # residual atomic charge
    effective_charges = jnp.asarray([z - n_cores.get(z, 0) for z in charges.tolist()])

    # mask to separate pseudo atoms from regular atoms
    ecp_mask = np.abs(np.asarray(charges) - effective_charges) > 2.0e-6
    # construct v_grids
    n_ecp = ecp_mask.sum()
    n_grid = r_grid.size
