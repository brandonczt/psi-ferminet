...
def _eval_mos(
    pos: jnp.ndarray, scf_approx: scf.Scf, nspins: Tuple[int, int]
...
def calc_hf_prob(
    ...
    _, occ_mos = _eval_mos(pos=pos.reshape(-1, 3), scf_approx=scf_approx, nspins=nspins)
...
def get_rho(
    ...
    for spin, i in enumerate(idx):
        ...
        if use_excited:
            ...
            log_max = jnp.maximum(
                jnp.max(denom_logs, axis=[1, 2], keepdims=True),
