...
def get_hf(
    ...
    scf_approx.run(excitations=max(states - 1, 0), excitation_type=excitation_type)
    return scf_approx


def eval_orbitals(
    scf_approx: scf.Scf, pos: Union[np.ndarray, jnp.ndarray], nspins: Tuple[int, int]
    ...
    if scf_fraction < 1e-6:
        ...
        def mcmc_network(full_params, pos, spins, atoms, charges):
            log_ferminet = batch_network(
                full_params["ferminet"], pos, spins, atoms, charges
            )
            log_scf = scf_network(full_params["scf"].eval_slater, pos)
            return (1 - scf_fraction) * log_ferminet + scf_fraction * log_scf

    mcmc_step = mcmc.make_mcmc_step(mcmc_network, batch_per_device=batch_size, steps=1)
    ...
    def loss_fn(
        ...
        if full_det:
            ...
            target = jnp.concatenate(
                (
                    jnp.concatenate((target[0], jnp.zeros(dims + (na, nb))), axis=-1),
                    jnp.concatenate((jnp.zeros(dims + (nb, na)), target[1]), axis=-1),
                ),
                axis=-2,
            )
